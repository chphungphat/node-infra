...
const defineRelationViewController = (opts) => {
    const { baseClass, relationType, relationName, defaultLimit = common_1.App.DEFAULT_QUERY_LIMIT, endPoint = '', schema } = opts;
            ...
            return __awaiter(this, void 0, void 0, function* () {
                ...
                switch (relationType) {
                    ...
                    case common_1.EntityRelations.HAS_MANY: {
                        ...
                        const targetRepository = yield ref.getTargetRepository();
                        return targetRepository.count(Object.assign(Object.assign({}, where), targetConstraint));
                    }
                    case common_1.EntityRelations.HAS_MANY_THROUGH: {
                        const throughConstraint = yield ref.getThroughConstraintFromSource();
                        const throughRepository = yield ref.getThroughRepository();
                        const thoughInstances = yield throughRepository.find({
                            where: Object.assign({}, throughConstraint),
                        });
                        const targetConstraint = yield ref.getTargetConstraintFromThroughModels(thoughInstances);
                        const targetModelName = yield ref.targetResolver().name;
                        const targetRepositoryGetter = (0, get_1.default)(ref.getTargetRepository, targetModelName);
...
const defineAssociateController = (opts) => {
    const { baseClass, relationName, defaultLimit = common_1.App.DEFAULT_QUERY_LIMIT, endPoint = '', schema } = opts;
    const restPath = `/{id}/${endPoint ? endPoint : relationName}`;
    const BaseClass = baseClass !== null && baseClass !== void 0 ? baseClass : common_2.BaseController;
...
const defineRelationCrudController = (controllerOptions) => {
    ...
    const { useControlTarget = true, defaultLimit = common_1.App.DEFAULT_QUERY_LIMIT, endPoint = '' } = options;
    const restPath = `{id}/${endPoint ? endPoint : relationName}`;
