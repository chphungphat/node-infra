...
const sqls = [
  ...
  FROM (
      SELECT
          pm.id as id,
          (CASE
              WHEN pm.user_id IS NOT NULL THEN '${constants_1.EnforcerDefinitions.PREFIX_USER}'
              WHEN pm.role_id IS NOT NULL THEN '${constants_1.EnforcerDefinitions.PREFIX_ROLE}'
              ELSE NULL
          END) AS subject_type,
          (CASE
          ...
          (CASE
              WHEN pm.user_id IS NOT NULL THEN
                  'p,${constants_1.EnforcerDefinitions.PREFIX_USER}_' || pm.user_id || ',' || LOWER(p.code) || ',${constants_1.EnforcerDefinitions.ACTION_EXECUTE},' || effect
              WHEN pm.role_id IS NOT NULL THEN
                  'p,${constants_1.EnforcerDefinitions.PREFIX_ROLE}_' || pm.role_id || ',' || LOWER(p.code) || ',${constants_1.EnforcerDefinitions.ACTION_EXECUTE},' || effect
              ELSE NULL
          END) AS policy
